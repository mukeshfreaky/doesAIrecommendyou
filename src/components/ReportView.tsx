    ...
    <div className="w-full max-w-5xl mx-auto space-y-12 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-800 pb-6">
        <div>
          <button
            ...
            className="inline-flex items-center gap-1.5 text-xs font-medium text-zinc-400 hover:text-zinc-100 transition-colors mb-3"
          >
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="text-sm font-mono text-zinc-300 px-2.5 py-0.5 rounded-md bg-zinc-900 border border-zinc-800">
              {report.domain}
            </span>
          </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-medium bg-zinc-900 hover:bg-zinc-800 text-zinc-200 border border-zinc-800 transition-colors"
          >
      ...
      <section className="space-y-4">
        ...
        {recommendedQuestions.length > 0 ? (
          ...
          <div className="rounded-xl border border-zinc-800 bg-zinc-950/70 p-6 text-center space-y-2">
      ...
      <section className="space-y-4">
        ...
        {losingOrLesserQuestions.length > 0 ? (
          ...
          <div className="rounded-xl border border-zinc-800 bg-zinc-950/70 p-6 text-center text-sm text-zinc-400">
      ...
      {/* ========================================================================= */}
      <section className="rounded-2xl border border-zinc-800 bg-zinc-950/70 p-6 md:p-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* 1. Observed Evidence */}
          <div className="p-4 rounded-xl bg-zinc-900/70 border border-zinc-800/80 space-y-2">
      ...
      {/* ========================================================================= */}
      <section className="rounded-2xl border border-zinc-800 bg-zinc-950/70 p-6 md:p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        ...
        <div className="shrink-0 flex items-center gap-3">
          <button
            ...
            className="px-5 py-3 rounded-xl text-xs font-semibold bg-blue-600 hover:bg-blue-500 text-white transition-all flex items-center gap-2 shadow-sm active:scale-95"
          >
      ...
      {notifyModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl max-w-md w-full p-6 space-y-4 animate-in fade-in zoom-in-95 duration-200 shadow-2xl">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-zinc-800 border border-zinc-700 flex items-center justify-center text-blue-400">
                <Compass className="w-5 h-5" />
              </div>
              <div>
                <h4 className="text-base font-bold text-zinc-100">Track AI Recommendation Progress</h4>
                <p className="text-xs text-zinc-400">Measure if your updates improve AI recommendations</p>
              </div>
            </div>
            <p className="text-xs text-zinc-300 leading-relaxed">
      ...
      <div className="border border-zinc-800 rounded-2xl bg-zinc-950/60 overflow-hidden">
        <button
          onClick={() => setTechDetailsOpen(!techDetailsOpen)}
          className="w-full px-6 py-4 flex items-center justify-between text-xs font-medium text-zinc-400 hover:text-zinc-200 transition-colors"
        >
        ...
        {techDetailsOpen && (
          <div className="px-6 pb-6 pt-2 border-t border-zinc-800/80 space-y-4 text-xs text-zinc-400">
            {/* Business Profile Metadata */}
            <div className="space-y-1.5">
              <div className="font-semibold text-zinc-100">Extracted Business Profile:</div>
              <div>
                <span className="text-zinc-500 font-medium">Pages Crawled:</span>{" "}
                {report.evidence.crawledPagesCount} pages ({report.evidence.sourcePages.join(", ")})
              </div>
              <div>
                <span className="text-zinc-500 font-medium">Detected Target Customers:</span>{" "}
                {report.businessProfile.targetCustomers.length > 0
                  ? report.businessProfile.targetCustomers.join(", ")
                  : "General business buyers"}
              </div>
              <div>
                <span className="text-zinc-500 font-medium">Pricing Signals:</span>{" "}
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <span className="text-zinc-500 font-medium">Retrieval Provider:</span>{" "}
                  <span className="font-mono text-zinc-300">
                    {report.providerMetadata.retrievalProvider || "Tavily Search API"}
                  </span>
                </div>
                <div>
                  <span className="text-zinc-500 font-medium">Evaluator Model:</span>{" "}
                  <span className="font-mono text-zinc-300">{report.providerMetadata.modelId}</span>
                </div>
                <div>
                  <span className="text-zinc-500 font-medium">Evidence Status:</span>{" "}
                  <span
                    className={
                      isEvidenceBacked
                ...
                <div>
                  <span className="text-zinc-500 font-medium">Retrieval Searches:</span>{" "}
                  {report.providerMetadata.retrievalQueriesExecuted || report.questions.length} queries
                </div>
                <div>
                  <span className="text-zinc-500 font-medium">Native Gemini Search:</span>{" "}
                  <span className="text-zinc-300 font-mono">0 (Disabled)</span>
                </div>
                <div>
                  <span className="text-zinc-500 font-medium">Latency:</span>{" "}
                  {Math.round(report.providerMetadata.latencyMs / 1000)}s
                </div>
                <div>
                  <span className="text-zinc-500 font-medium">Tokens:</span>{" "}
